...
$(document).ready(function(){
    ...
    let replyRegDate = $('#walkCommendRd').val();

    $('.walk-date-register').text(date.formatDates(regDate));
    $('.insert-modDate').text(date.formatDates(modDate));
    $('.reply-reg-date').text(date.formatDates(replyRegDate));
...
function initMap() {
    window.kakao.maps.load(() => {
        ...
        //주소 정보 가져오기
        const retrievedAddress = $('#fullAddress').text();
